...
export const Generators = {
    ...
    Castle: (): VoxelData[] => {
        ...
        // 4 Towers
        const towers = [[-SIZE, -SIZE], [SIZE, -SIZE], [-SIZE, SIZE], [SIZE, SIZE]];
            ...
            for(let y=0; y<HEIGHT; y++) {
                for(let x=-3; x<=3; x++) {
                    for(let z=-3; z<=3; z++) {
                        // Round towers
                        if (x*x + z*z < 8) {
                            setBlock(map, tx+x, FLOOR+y, tz+z, COLORS.WHITE);
                        }
                    }
                }
            }
            ...
            for(let y=0; y<5; y++) {
                 for(let x=-4+y; x<=4-y; x++) {
                    for(let z=-4+y; z<=4-y; z++) {
                         setBlock(map, tx+x, FLOOR+HEIGHT+y, tz+z, COLORS.LIGHT);
                    }
                 }
            }
        });
        ...
        for (let y=0; y<HEIGHT*0.7; y++) {
            for (let i=-SIZE; i<=SIZE; i++) {
                setBlock(map, i, FLOOR+y, -SIZE, COLORS.WHITE); // Back
                setBlock(map, i, FLOOR+y, SIZE, COLORS.WHITE);  // Front
                setBlock(map, -SIZE, FLOOR+y, i, COLORS.WHITE); // Left
                setBlock(map, SIZE, FLOOR+y, i, COLORS.WHITE);  // Right
            }
        }
             ...
             for(let x=-6; x<=6; x++) {
                for(let z=-6; z<=6; z++) {
                    if (Math.abs(x) > 4 || Math.abs(z) > 4) {
                         setBlock(map, x, FLOOR+y, z, COLORS.DARK); // Hollow box
                    }
                }
             }
        }
        ...
        for(let y=0; y<6; y++) {
            for(let x=-3; x<=3; x++) {
                 setBlock(map, x, FLOOR+y, SIZE+1, COLORS.WOOD);
            }
        }
